import tree_sitter_cpp as tscpp
from tree_sitter import Language, Parser

CPP_LANGUAGE = Language(tscpp.language())
parser = Parser(CPP_LANGUAGE)

def dump(node, source_bytes, depth=0):
    text_preview = source_bytes[node.start_byte:node.end_byte].decode("utf8").split("\n")[0][:40]
    print("  " * depth + f"{node.type}   -> {text_preview!r}")
    for child in node.children:
        dump(child, source_bytes, depth + 1)

print("=" * 15, "plain call: helper()", "=" * 15)
s1 = b"void caller() { helper(); }"
t1 = parser.parse(s1)
dump(t1.root_node, s1)

print()
print("=" * 15, "this->method()", "=" * 15)
s2 = b"void Widget::render() { this->helperMethod(); }"
t2 = parser.parse(s2)
dump(t2.root_node, s2)

print()
print("=" * 15, "field check on this->method()", "=" * 15)
def find_first(node, type_name):
    if node.type == type_name:
        return node
    for child in node.children:
        found = find_first(child, type_name)
        if found:
            return found
    return None

call_node = find_first(t2.root_node, "call_expression")
print(f"call node type: {call_node.type if call_node else None}")
if call_node:
    func_field = call_node.child_by_field_name("function")
    print(f"function field: {func_field}, type={func_field.type if func_field else None}")
    if func_field and func_field.type == "field_expression":
        print(f"  argument field: {func_field.child_by_field_name('argument')}")
        print(f"  field field: {func_field.child_by_field_name('field')}")